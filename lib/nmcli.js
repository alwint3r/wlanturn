...
module.exports = {
  listAp(callback) {
    ...
    const result = pipeline(command);

    if (result.length < 1) {
      return callback(
        new Error(`Can't find any access points. Is the interface down?`)
      );
    }
  ...
  connect(ssid, password, force) {
    function newConnection() {
      return exec(`nmcli d wifi connect "${ssid}" password "${password}"`);
    }

    if (!force && exec(`nmcli c list id "${ssid}"`).code === 0) {
      return exec(`nmcli c up id "${ssid}"`).code === 0;
    }

    return newConnection().code === 0;
  },
